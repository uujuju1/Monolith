			...
			generator = new MonolithPlanetGenerator() {{
				...
				gen = p -> {
					...
					for (Room room : rooms) {
						...
						p.brush(p.pathfind(room.x, room.y, room.connect.x, room.connect.y, tile -> Mathf.dst(tile.x, tile.y, room.connect.x, room.connect.y), Astar.manhattan, tile -> true), rand().random(10, 20));
					}
				};
			}};
